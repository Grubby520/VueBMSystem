             ...
             <div class="flex-layout flex-item-row flex-space-1 content-statistic-module">
               ...
               <div class="flex-layout flow-content">
                 <div class="flex-space-1">
                   ...
                   <span class="flow-text">CPU<base-label :type="cpuStatus.sts" v-text="cpuStatus.txt"></base-label></span>
                 </div>
                 <div class="flex-space-1">
                   <div class="flow-num danger" id="load-rvm">?</div>
                   <span class="flow-text">内存<base-label :type="memoryStatus.sts" v-text="memoryStatus.txt"></base-label></span>
                 </div>
                 <div class="flex-space-1 warm">
                   <div class="flow-num" id="load-storage">?</div>
                   <span class="flow-text">储存<base-label :type="storageStatus.sts" v-text="storageStatus.txt"></base-label></span>
                 </div>
               </div>
             </div>
          </div>
          ...
          <w-row>
            ...
            <div slot="content">
              <table >
                ...
                <tbody>
                  <tr>
                    <td>CPU负载<span class="tip-info" :class="cpuStatus.sts" v-text="cpuStatus.txt">高</span>，平均负载<span  class="tip-info" :class="cpuStatus.sts" v-text="vmInfo.cpuLoad?vmInfo.cpuLoad*100+'%':'?%'">33</span></td>
                    <td><base-label :type="cpuStatus.sts" v-text="vmInfo.cpuSuggest">优化</base-label>
                      <template v-if="cpuStatus.sts !='success'">
                        推荐CPU配置（个）：<span v-text="vmInfo.recommendCpu?vmInfo.recommendCpu:'?'"></span>，预计负载：<span v-text="vmInfo.estimateCpuLoad?vmInfo.estimateCpuLoad*100+'%':'?'"></span>
                      </template>
                    </td>
                  </tr>
                  <tr>
                    <td>内存负载<span class="tip-info" :class="memoryStatus.sts" v-text="memoryStatus.txt">高</span>，平均负载<span class="tip-info" :class="memoryStatus.sts" v-text="vmInfo.memoryLoad?vmInfo.memoryLoad*100+'%':'?%'">33</span></td>
                    <td><base-label :type="memoryStatus.sts" v-text="vmInfo.memorySuggest">优化</base-label>
                      <template v-if="memoryStatus.sts !='success'">
                        推荐内存配置（GB）：<span v-text="vmInfo.recommendMemory?vmInfo.recommendMemory:'?'"></span>，预计负载：<span v-text="vmInfo.estimateMemoryLoad?vmInfo.estimateMemoryLoad*100+'%':'?'"></span>
                      </template>
                    </td>
                  </tr>
                  <tr>
                    <td>存储负载<span class="tip-info" :class="storageStatus.sts" v-text="storageStatus.txt">高</span>，平均负载<span class="tip-info" :class="storageStatus.sts" v-text="vmInfo.storageLoad?vmInfo.storageLoad*100+'%':'?%'">33</span></td>
                    <td><base-label :type="storageStatus.sts" v-text="vmInfo.storageSuggest">优化</base-label>
                      <template v-if="storageStatus.sts !='success'">
                        推荐存储配置（GB）：<span v-text="vmInfo.recommendStorage?vmInfo.recommendStorage:'?'"></span>，预计负载：<span v-text="vmInfo.estimateStorageLoad?vmInfo.estimateStorageLoad*100+'%':'?'"></span>
                      </template>
                    </td>
                  </tr>
                </tbody>
